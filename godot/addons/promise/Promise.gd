...
func _init(callable: Callable):
	resolved.connect(
		func(value: Variant):
			settled.emit(PromiseResult.new(eStatus.RESOLVED, value)),
		CONNECT_ONE_SHOT
	)
	rejected.connect(
		func(rejection: Rejection):
			settled.emit(PromiseResult.new(eStatus.REJECTED, rejection)),
		CONNECT_ONE_SHOT
	)
	settled.connect(
...
func then(resolved_callback: Callable) -> Promise:
	if is_settled and last_result and last_result.status == Promise.eStatus.RESOLVED:
...
func catch(rejected_callback: Callable) -> Promise:
	if is_settled and last_result and last_result.status == Promise.eStatus.REJECTED:
...
class PromiseResult:
	...
	func _init(_status: eStatus, _payload: Variant):
		status = _status
		payload = _payload
